...
void EventStreamParser::DetectArtificialRotateChecksum() {
  if (has_checksum_ || current_has_checksum_ ||
  ...
  // here. The framing of the buffered event was frozen when its header was
  // parsed, and has_checksum_ may have moved since.
  if (computed == stored) current_has_checksum_ = true;
}
