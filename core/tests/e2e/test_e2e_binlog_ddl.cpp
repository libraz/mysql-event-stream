...
TEST(E2EBinlogDDL, CreateTableDuringStream) {
  ...
  ASSERT_EQ(ExecuteDML("CREATE TABLE mes_test.dyn_create_test ("
                       ...
                       "name VARCHAR(50))"),
            MES_OK);
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.dyn_create_test (id, name) VALUES (1, 'hello')"),
            MES_OK);
...
TEST(E2EBinlogDDL, AlterTableAddColumn) {
  ...
  ASSERT_EQ(ExecuteDML("ALTER TABLE mes_test.ddl_test ADD COLUMN extra INT DEFAULT 0"), MES_OK);
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.ddl_test (val, extra) VALUES ('added', 42)"), MES_OK);
...
TEST(E2EBinlogDDL, AlterTableDropColumn) {
  ...
  ASSERT_EQ(ExecuteDML("ALTER TABLE mes_test.ddl_test DROP COLUMN temp_col"), MES_OK);
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.ddl_test (val) VALUES ('dropped')"), MES_OK);
...
TEST(E2EBinlogDDL, AlterTableModifyType) {
  ...
  ASSERT_EQ(ExecuteDML("ALTER TABLE mes_test.ddl_test MODIFY val TEXT"), MES_OK);
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.ddl_test (val) VALUES ('modified_type_test')"),
            MES_OK);
...
TEST(E2EBinlogDDL, DropTableDuringStream) {
  ...
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.ddl_test (val) VALUES ('after_drop')"), MES_OK);
  ...
  ASSERT_GE(ddl_events.size(), 1u) << "Stream should continue after DROP TABLE";
...
TEST(E2EBinlogDDL, RenameTableDuringStream) {
  ...
  ASSERT_EQ(ExecuteDML("RENAME TABLE mes_test.ddl_test TO mes_test.ddl_test_renamed"), MES_OK);
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.ddl_test_renamed (val) VALUES ('renamed')"), MES_OK);
...
TEST(E2EBinlogDDL, TruncateTable) {
  ...
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.ddl_test (val) VALUES ('before_truncate')"), MES_OK);

  std::string gtid = GetCurrentGtid();
  ASSERT_FALSE(gtid.empty()) << "Failed to fetch current GTID";

  ASSERT_EQ(ExecuteDML("TRUNCATE TABLE mes_test.ddl_test"), MES_OK);
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.ddl_test (val) VALUES ('after_truncate')"), MES_OK);

  auto events = CaptureTableEvents(gtid, 406, "ddl_test", 1);
  auto filtered = FilterByTable(events, "ddl_test");

  ASSERT_GE(filtered.size(), 1u) << "Expected at least 1 INSERT after TRUNCATE";
  EXPECT_EQ(filtered[0].type, MES_EVENT_INSERT);
}
...
TEST(E2EBinlogDDL, DmlAfterMultipleSchemaChanges) {
  ...
  ASSERT_EQ(ExecuteDML("ALTER TABLE mes_test.ddl_test ADD COLUMN c1 INT DEFAULT 0"), MES_OK);
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.ddl_test (val, c1) VALUES ('sc1', 10)"), MES_OK);

  // Schema change 2: add c2
  ASSERT_EQ(ExecuteDML("ALTER TABLE mes_test.ddl_test ADD COLUMN c2 INT DEFAULT 0"), MES_OK);
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.ddl_test (val, c1, c2) VALUES ('sc2', 20, 30)"),
            MES_OK);

  // Schema change 3: drop c1
  ASSERT_EQ(ExecuteDML("ALTER TABLE mes_test.ddl_test DROP COLUMN c1"), MES_OK);
  ASSERT_EQ(ExecuteDML("INSERT INTO mes_test.ddl_test (val, c2) VALUES ('sc3', 40)"), MES_OK);

  // Capture all 3 INSERTs
  auto events = CaptureTableEvents(gtid, 407, "ddl_test", 3);
  auto filtered = FilterByTable(events, "ddl_test");

  ASSERT_GE(filtered.size(), 3u) << "Expected 3 INSERTs for ddl_test after multiple schema changes";

  // 1st INSERT: id, val, c1 (3 columns)
  EXPECT_EQ(filtered[0].type, MES_EVENT_INSERT);
  EXPECT_EQ(filtered[0].after.size(), 3u) << "1st INSERT should have 3 columns (id, val, c1)";

  // 2nd INSERT: id, val, c1, c2 (4 columns)
  EXPECT_EQ(filtered[1].type, MES_EVENT_INSERT);
  EXPECT_EQ(filtered[1].after.size(), 4u) << "2nd INSERT should have 4 columns (id, val, c1, c2)";
