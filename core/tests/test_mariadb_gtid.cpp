...
TEST(MariaDBGtidTest, ParseLargeValues) {
  MariaDBGtid gtid;
  ASSERT_EQ(MES_OK, MariaDBGtid::Parse("100-4294967295-18446744073709551615", &gtid));
...
TEST(MariaDBGtidTest, ParseMySQLGtidFormat) {
  ...
  EXPECT_EQ(MES_ERR_INVALID_ARG,
            MariaDBGtid::Parse("3e11fa47-71ca-11e1-9e33-c80aa9429562:1-77", &gtid));
}
...
TEST(MariaDBGtidTest, ParseDomainIdOverflow) {
  ...
  EXPECT_EQ(MES_ERR_INVALID_ARG, MariaDBGtid::Parse("4294967296-1-42", &gtid));
}

TEST(MariaDBGtidTest, ParseNullOutput) { EXPECT_NE(MES_OK, MariaDBGtid::Parse("0-1-42", nullptr)); }
...
TEST(MariaDBGtidTest, ParseSetThreeGtids) {
  std::vector<MariaDBGtid> gtids;
  ASSERT_EQ(MES_OK, MariaDBGtid::ParseSet("0-1-42,1-2-100,2-3-200", &gtids));
...
TEST(MariaDBGtidTest, ParseSetInvalidEntry) {
  std::vector<MariaDBGtid> gtids;
  EXPECT_EQ(MES_ERR_INVALID_ARG, MariaDBGtid::ParseSet("0-1-42,invalid", &gtids));
}
...
TEST(MariaDBGtidTest, SetToStringEmpty) { EXPECT_EQ(MariaDBGtid::SetToString({}), ""); }
...
TEST(MariaDBGtidTest, IsMariaDBFormatEmpty) { EXPECT_FALSE(MariaDBGtid::IsMariaDBGtidFormat("")); }
...
TEST(BinlogEventTypeTest, MariaDBEventValues) {
  ...
  EXPECT_EQ(static_cast<uint8_t>(BinlogEventType::kMariaDBAnnotateRowsEvent), 160);
  EXPECT_EQ(static_cast<uint8_t>(BinlogEventType::kMariaDBBinlogCheckpointEvent), 161);
  EXPECT_EQ(static_cast<uint8_t>(BinlogEventType::kMariaDBGtidEvent), 162);
  EXPECT_EQ(static_cast<uint8_t>(BinlogEventType::kMariaDBGtidListEvent), 163);
  EXPECT_EQ(static_cast<uint8_t>(BinlogEventType::kMariaDBStartEncryptionEvent), 164);
}
